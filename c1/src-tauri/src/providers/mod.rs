...
pub struct ProviderInfo {
    ...
    /// True if this provider cannot filter sessions by project (shows all projects)
    pub is_global: bool,
}

/// Aggregate token usage for a provider
...
pub trait SessionProvider {
    ...
    /// Aggregate token usage across all sessions for the given project.
    /// Default implementation returns None (provider doesn't track tokens).
...
pub fn detect_providers(project_path: &str) -> Vec<ProviderInfo> {
    ...
    // Gemini CLI
    let gem = gemini::GeminiProvider;
    if let Ok(info) = gem.info(project_path) {
        providers.push(info);
    }
...
pub fn get_provider(kind: ProviderKind) -> Box<dyn SessionProvider + Send> {
    match kind {
        ...
        ProviderKind::GeminiCli => Box::new(gemini::GeminiProvider),
    }
}
