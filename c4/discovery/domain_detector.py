...
class DomainDetector:
    ...
    DETECTION_RULES: list[tuple[str, Optional[str], Domain, float, str]] = [
        ...
        (
            "pyproject.toml", r"\[build-system\]", Domain.LIBRARY, 0.75,
    ...
    def _check_rule(
        self, file_pattern: str, content_pattern: Optional[str]
    ) -> list[str]:
    ...
    def infer_domain_from_description(self, description: str) -> Domain:
        ...
        patterns: dict[Domain, list[str]] = {
            Domain.WEB_FRONTEND: [
                "website", "web app", "웹앱", "웹사이트", "react", "vue", "frontend",
                "프론트엔드", "ui", "사용자 인터페이스", "landing page", "랜딩",
                "dashboard", "대시보드", "spa", "웹 페이지",
            ],
            Domain.WEB_BACKEND: [
                "api", "server", "서버", "backend", "백엔드", "rest", "graphql",
                "microservice", "마이크로서비스", "endpoint", "database",
            ],
            Domain.ML_DL: [
                "model", "모델", "machine learning", "머신러닝", "deep learning",
                "딥러닝", "neural", "뉴럴", "training", "학습", "prediction", "예측",
                "classification", "분류", "regression", "회귀", "nlp", "cv",
                "computer vision", "컴퓨터 비전", "transformer", "gpt", "bert",
                "embedding", "임베딩", "inference", "추론",
            ],
            Domain.MOBILE_APP: [
                "app", "앱", "mobile", "모바일", "ios", "android", "안드로이드",
                "flutter", "react native", "swift", "kotlin", "휴대폰",
            ],
            Domain.INFRA: [
                "infrastructure", "인프라", "deploy", "배포", "kubernetes", "k8s",
                "docker", "terraform", "aws", "gcp", "azure", "cloud", "클라우드",
                "ci/cd", "pipeline", "파이프라인", "devops",
            ],
            Domain.LIBRARY: [
                "library", "라이브러리", "package", "패키지", "sdk", "module",
                "모듈", "utility", "유틸리티", "framework", "프레임워크",
            ],
            Domain.FIRMWARE: [
                "firmware", "펌웨어", "embedded", "임베디드", "microcontroller",
                "마이크로컨트롤러", "arduino", "stm32", "esp32", "iot",
            ],
        }
