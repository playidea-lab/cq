"""MR Review Service - AI-powered code review for GitLab.

This service analyzes MR diffs using Claude API and generates
structured code review feedback. Mirrors PRReviewService for GitLab.
"""
...
from c4.integrations.gitlab_client import GitLabClient, MRInfo
...
REVIEW_SYSTEM_PROMPT = """You are an expert code reviewer. Analyze the provided MR diff and give constructive feedback.
...
class MRReviewService:
    """Service for AI-powered MR code review on GitLab.

    This service coordinates between the GitLab client and the LLM
    to provide automated code reviews for merge requests.
    """
    ...
    async def review_mr(self, mr_info: MRInfo) -> ReviewResult:
        """Review an MR and post comments.

        Args:
            mr_info: MR information from webhook

        Returns:
            ReviewResult with status
        """
        # Get diff
        diff = self.gitlab_client.get_mr_diff(mr_info)
        if not diff:
            return ReviewResult(
                success=False,
                message="Failed to retrieve MR diff",
            )

        # Check size
        if len(diff) > self.max_diff_size:
            logger.warning(f"MR diff too large: {len(diff)} bytes (max: {self.max_diff_size})")
            return ReviewResult(
                success=False,
                message=f"MR diff too large ({len(diff)} bytes). Max: {self.max_diff_size} bytes",
            )

        # Analyze with LLM
        analysis = await self._analyze_diff(diff, mr_info)
        if not analysis:
            return ReviewResult(
                success=False,
                message="Failed to analyze MR diff",
            )

        # Post review as note
        review_body = self._format_review_body(analysis)
        result = self.gitlab_client.create_mr_note(mr_info, review_body)

        # Post line comments as discussions if there are issues with line numbers
        if result.success:
            self._post_line_comments(mr_info, analysis)

            # Add labels if suggested
            if analysis.labels_to_add:
                self.gitlab_client.add_mr_labels(mr_info, analysis.labels_to_add)
    ...
    def _post_line_comments(self, mr_info: MRInfo, analysis: ReviewAnalysis) -> None:
        """Post line-specific comments as discussions.

        Args:
            mr_info: MR information
            analysis: Review analysis with issues
        """
        for issue in analysis.issues:
            ...
            # Note: GitLab requires specific position info for line comments
            # This is a simplified version - full implementation would need
            # base_sha and head_sha from the MR
            try:
                self.gitlab_client.create_discussion(
                    mr_info,
                    body,
                    position={
                        "position_type": "text",
                        ...
                        "new_line": issue.line,
                        "base_sha": "",  # Would need to be filled
                        "head_sha": mr_info.head_sha,
                        "start_sha": "",  # Would need to be filled
                    },
                )
            except Exception as e:
                logger.warning(f"Failed to post line comment: {e}")

    async def _analyze_diff(self, diff: str, mr_info: MRInfo) -> ReviewAnalysis | None:
        """Analyze diff using LLM.

        Args:
            diff: MR diff content
            mr_info: MR information for context

        Returns:
            ReviewAnalysis or None on error
        """
        user_prompt = f"""Review this merge request:

**Title:** {mr_info.title}
**Author:** {mr_info.author}
**Target:** {mr_info.target_branch} <- **Source:** {mr_info.source_branch}
...
Provide a thorough code review in JSON format."""
    ...
    async def _call_anthropic(self, prompt: str) -> str:
        ...
        try:
            ...
            raise ImportError(
                "Either 'litellm' or 'anthropic' package is required for MR review. "
    ...
    def _format_review_body(self, analysis: ReviewAnalysis) -> str:
        ...
        # Quality badge - using GitLab-compatible emoji
        quality_emoji = {
            "good": ":white_check_mark:",
            "acceptable": ":warning:",
            "needs_work": ":x:",
        }
        emoji = quality_emoji.get(analysis.overall_quality or "acceptable", ":warning:")
        ...
        if analysis.issues:
            ...
            body += "### Issues Summary\n\n"
            if critical:
                body += f"- :x: Critical: {critical}\n"
            if warnings:
                body += f"- :warning: Warnings: {warnings}\n"
            if suggestions:
                body += f"- :information_source: Suggestions: {suggestions}\n"
            if praise:
                body += f"- :star: Praise: {praise}\n"
            body += "\n"
