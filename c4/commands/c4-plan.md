...
## ⚠️ 핵심 원칙 2: 메인 세션은 계획만, 실행은 Worker가

> **절대 규칙**: 메인 세션(Plan 모드)은 태스크를 생성만 하고, 직접 구현하지 않습니다.

```
❌ 금지:
- 태스크 추가 후 "직접 하겠습니다" → 코드 작성
- c4_add_todo() 후 바로 파일 수정
- Worker 없이 메인이 구현 작업 수행

✅ 필수:
- 태스크 생성 → /c4-run으로 Worker 스폰 → Worker가 실행
- 메인은 모니터링, 리뷰, 의사결정에 집중
- 구현이 필요하면 반드시 Worker를 통해 실행
```
...
**구현 작업은 Worker에게:**
```bash
/c4-run           # Worker 스폰 → Worker가 태스크 처리
/c4-run 3         # 3개 Worker 병렬 스폰
```

**위반 시**: `c4_submit()` 실패, Worker와 작업 충돌 발생.
