...
func testWorkerDeps(t *testing.T, hubHandler http.HandlerFunc, keeperHandler http.HandlerFunc) (*WorkerDeps, *httptest.Server) {
	...
	if keeperHandler != nil {
		...
		c1 := messengerhandler.NewC1Handler(keeperServer.URL, "test-key", cloud.NewStaticTokenProvider("test-token"), "proj-1")
		deps.Keeper = messengerhandler.NewContextKeeper(c1, nil)
	}
