...
@pytest.fixture
def mock_llm():
    """Create a mock LLMClient that returns sequential responses."""
    from c4.review.llm import LLMResponse

    mock = MagicMock()
    call_count = [0]

    def chat_side_effect(messages, max_tokens=4096):
        call_count[0] += 1
        if call_count[0] == 1:
            ...
            return LLMResponse(text="Generated review content in Korean\n\n---\n\nEnglish translation")

    mock.chat = MagicMock(side_effect=chat_side_effect)
    mock.format_image = MagicMock(side_effect=lambda b64, mt="image/png": {"type": "image", "source": {"type": "base64", "media_type": mt, "data": b64}})
    mock.format_text = MagicMock(side_effect=lambda t: {"type": "text", "text": t})
