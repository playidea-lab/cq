...
│   ├── POST   /{id}/stop     # 실행 중단
│   ├── GET    /{id}/files    # 파일 목록
│   ├── GET    /{id}/download # 결과물 다운로드
│   └── WS     /{id}/events   # 실시간 이벤트 (SSE/WebSocket)
│
├── chat/
│   └── POST /message         # 채팅 메시지 (SSE 스트림 응답)
│
├── billing/
│   ├── GET  /balance         # 잔액 조회
│   ├── POST /deposit         # 충전 (Stripe)
│   ├── GET  /transactions    # 거래 내역
│   └── GET  /usage           # 사용량 조회
│
├── github/
│   ├── GET  /repos           # 연결된 저장소 목록
│   ├── POST /connect         # 저장소 연동
│   └── POST /{id}/push       # 결과물 Push (PR 생성)
│
└── workers/
    ├── GET  /                # 워커 상태 목록
    └── POST /{project_id}/scale  # 워커 스케일 조정
```
...
type EventType =
  | "task_started"      // 태스크 시작
  | "task_progress"     // 태스크 진행 중 (스트리밍)
  | "task_completed"    // 태스크 완료
  | "checkpoint"        // 체크포인트 도달
  | "review_started"    // 리뷰 시작
  | "review_completed"  // 리뷰 완료 (approve/reject)
  | "usage_update"      // 토큰 사용량 업데이트
  | "github_pushed"     // GitHub PR 생성됨
...
| 항목 | 구현 | 설명 |
|------|------|------|
| **토큰 암호화** | AES-256 | GitHub 토큰, API 키 암호화 저장 |
| **Rate Limiting** | per-user, per-IP | API 악용 방지 |
| **입력 검증** | Pydantic | 모든 API 입력 검증 |
| **격리 실행** | gVisor + tmpfs | 프로젝트별 완전 격리 |
| **네트워크 제한** | Egress allowlist | GitHub, npm, pypi만 허용 |
| **감사 로그** | 모든 API 호출 | 보안 이벤트 추적 |
| **세션 관리** | JWT + Refresh | 토큰 만료 및 갱신 |
| **CORS** | Origin 제한 | 허용된 도메인만 |
...
|--------|------|----------------|
| Vercel Pro | 프론트엔드 | $20 |
| Fly.io | API 서버 + 워커 | $50-150 |
| Supabase Pro | PostgreSQL + Auth + Realtime | $25 |
| Cloudflare R2 | 결과물 스토리지 | $5-20 |
| Stripe | 결제 수수료 | 2.9% + $0.30/건 |
| Sentry | 에러 추적 | $26 |
| **합계** | | **$130-250** |

**손익분기점 예상**: 월 $500 이상 LLM 사용량 발생 시 (+20% 마크업 기준)
