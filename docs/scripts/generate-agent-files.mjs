...
const skills = [
  ...
  { name: 'c4-finish',     trigger: ['/c4-finish', '마무리', 'finish', '완료'],           description: 'Build → test → docs → commit. Post-implementation routine.', tier: 'all', category: 'workflow' },
  ...
  // Quality loop
  { name: 'c4-polish',     trigger: ['/c4-polish', 'polish'],                            description: '[DEPRECATED] Built into /c4-finish. No separate invocation needed.', tier: 'all', category: 'quality', deprecated: true },
  { name: 'c4-refine',     trigger: ['/c4-refine', 'refine'],                            description: '[DEPRECATED] Built into /c4-finish. No separate invocation needed.', tier: 'all', category: 'quality', deprecated: true },
  ...
  // Research & docs
  { name: 'c2-paper-review', trigger: ['논문 리뷰', 'paper review', '/c2-paper-review'], description: '[DEPRECATED] Use /c4-review instead.', tier: 'all', category: 'research', deprecated: true },
  ...
  { name: 'c4-attach',     trigger: ['/c4-attach', '세션 이름', 'attach', 'name this session'], description: 'Attach a name to the current session for later resume with `cq claude -t <name>`. Optionally add a memo.', tier: 'all', category: 'utility' },
  { name: 'c4-reboot',     trigger: ['/c4-reboot', 'reboot', '재시작', 'restart session'], description: 'Reboot current named session. cq resumes the same session UUID automatically.', tier: 'all', category: 'utility' },
]
