...
| Skill | 트리거 | 설명 |
|-------|--------|------|
| `/pi` | play idea, ideation, /pi | 계획 전 아이디어를 브레인스토밍하고 다듬습니다. 발산/수렴/조사/토론 모드. `idea.md`를 작성하고 자동으로 `/plan`을 실행합니다. |
...
| Skill | 트리거 | 설명 |
|-------|--------|------|
| `/plan` | plan, design, spec | Discovery -> Design -> Lighthouse 계약 -> 태스크 생성. 기능을 위한 완전한 구조화된 계획. |
| `/run` | run, execute | 모든 대기 중인 태스크를 위한 Worker 병렬 스폰. 지속 모드 — 큐가 빌 때까지 자동 재스폰. |
| `/finish` | finish, complete | 빌드 -> 테스트 -> 문서 -> 커밋. 구현 후 완료 루틴. |
| `/status` | status | 진행률, 의존성 그래프, 큐 요약, Worker 상태가 있는 시각적 태스크 그래프. |
| `/quick` | quick | 태스크 즉시 생성 + 할당, 계획 건너뜀. 작고 집중된 변경사항용. |
...
| Skill | 트리거 | 설명 |
|-------|--------|------|
| `/checkpoint` | (체크포인트에서 자동) | 단계 게이트: 4렌즈 리뷰 (전체적 / 사용자 흐름 / 파급 효과 / 출시 준비). 승인, 변경 요청, 재계획, 재설계. |
| `/validate` | validate | lint + 테스트 실행. CRITICAL은 커밋 차단, HIGH는 리뷰 필요, MEDIUM은 권장. |
| `/review` | review | 6축 평가를 통한 종합적 3-pass 코드 또는 논문 리뷰. 공식 리뷰 문서 생성. |
| `/polish` | polish | *(Deprecated — polish 루프가 `/finish`에 내장됨)* |
| `/refine` | refine | *(Deprecated — 품질 루프가 `/finish`에 내장됨)* |
...
| Skill | 트리거 | 설명 |
|-------|--------|------|
| `/add-task` | add task | DoD, 범위, 도메인 가이던스와 함께 태스크 대화형 추가. 기존 패턴에서 ID를 추론. |
| `/submit` | submit | 자동 유효성 검사와 함께 완료된 태스크 제출. 커밋 SHA 확인, 필요 시 체크포인트 트리거. |
...
| `/c2-paper-review` | paper review | *(Deprecated — 대신 `/review` 사용)* |
...
| `/validate` | 품질 루프 |
| `/review` | 품질 루프 |
| `/polish` | 품질 루프 (deprecated) |
| `/refine` | 품질 루프 (deprecated) |
| `/add-task` | 태스크 관리 |
| `/submit` | 태스크 관리 |
| `/c4-interview` | 태스크 관리 |
| `/c4-stop` | 태스크 관리 |
