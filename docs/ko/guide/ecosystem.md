...
```
Drive      — 클라우드 파일 스토리지 (Supabase Storage)
Docs       — 문서 라이프사이클 (파싱, 워크스페이스, 프로필)
EventBus   — gRPC 이벤트 버스 (UDS + WebSocket + DLQ)
Engine     — MCP 오케스트레이션 엔진  ← 여기
Hub        — Supabase 네이티브 워커 큐 (pgx LISTEN/NOTIFY, lease 기반)
Relay      — WSS NAT 관통 + 로컬 프록시 (토큰 없는 .mcp.json)
Guard      — RBAC 접근 제어 (정책, 감사, 역할 할당)
Observe    — 관측 가능성 레이어 (메트릭, 로그, 트레이싱 미들웨어)
Gate       — 외부 통합 (웹훅, 스케줄러, 커넥터)
Knowledge  — 지식 관리 (FTS5 + pgvector + 임베딩 + 수집)
```
...
| full | 전체 컴포넌트 (+ Hub 워커 큐, Relay, Guard, Observe, Gate) |

Guard, Observe, Gate는 빌드 태그로 활성화됩니다 — `full` 티어 바이너리에는 항상 컴파일되어 있습니다.

---

## C4 Engine (이 프로젝트)

Engine은 오케스트레이션 코어입니다. Claude Code에 **169개 이상의 MCP 도구 (티어별로 다름)** (`c4_*`)를 노출하고 다음을 관리합니다:
...
- **스킬** — 바이너리에 내장된 39개 슬래시 명령 (/pi, c9-* 연구 루프 등)
- **Relay 프록시** — 원격 워커용 로컬 프록시 (`.mcp.json`에 토큰 없음)
...
NAT 뒤 원격 워커를 위한 관통 레이어:

- **WSS 브릿지** — NAT 뒤 워커가 `cq-relay.fly.dev`에 아웃바운드 연결; 인바운드 포트 불필요
- **로컬 프록시** — `cq serve`가 `localhost:4140/w/{worker}/mcp` 노출; 매 요청마다 JWT 자동 주입
- **토큰 없는 `.mcp.json`** — worker 항목에 Bearer 토큰 없음 (커밋 가능)
- **자동 재연결** — 네트워크 끊김 시 지수 백오프, 토큰 만료 시 자동 갱신
- **WSL2 지원** — Windows Task Scheduler 자동 등록, nvidia-smi PATH fallback
- **워커 생존성** — systemd `Restart=always`, macOS `KeepAlive`, OS 부팅 시 자동 시작
...
Claude Code
    │
    ├── MCP (stdio) ──► Engine ──── Knowledge (검색 + 기록)
    │                     │                    ▲
    │                     ├── EventBus ────────┘ (task.completed → 자동 기록)
    │                     ├── Supabase (LISTEN/NOTIFY 워커 큐)
    │                     │       └── Drive를 통한 아티팩트 스토리지
    │                     └── LLM Gateway (Anthropic / OpenAI / Gemini / Ollama)
    │
    └── MCP (http) ──► cq serve (localhost:4140)
                          ├── Relay 프록시 ──WSS──► Relay (Fly.io) ──► GPU 워커
                          └── 토큰 자동 주입 (.mcp.json에 시크릿 없음)
```
