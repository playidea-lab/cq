...
워커가 자동으로 시작됩니다 — 태스크당 하나씩, 각자 격리된 git 워크트리에서 실행됩니다. 각 워커는 내장된 **polish 루프** (리뷰 → 수정 → 수렴)를 거친 후 제출합니다. Go 레벨 polish 게이트가 리뷰되지 않은 코드를 거부하므로 (diff ≥ 5줄), 품질은 시스템이 보장합니다.
...
| 게이트 | 시점 | 확인 내용 |
|--------|------|----------|
| **Refine** | `/c4-plan`에서 4개+ 태스크 추가 | critique 루프 필수 |
| **Polish** | 워커가 코드 제출 (diff ≥ 5줄) | 자체 리뷰 수렴 필수 |
| **Review** | 모든 구현 후 | 6축 평가 (정확성, 보안, 신뢰성, 관측성, 테스트, 가독성) |

이것은 스킵할 수 없는 Go 레벨 체크입니다. CQ를 많이 쓸수록 리뷰가 날카로워집니다 — [페르소나 온톨로지](/ko/guide/ecosystem)가 태스크 전반의 선호도를 학습합니다.
