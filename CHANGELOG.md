...
- **standards**: paper-review + paper-writing 스킬 추가 (연구팀)
...
### 📚 Documentation
- **docs**: GitHub Pages 업데이트 — `cq doctor --fix` 자동 설정, worker setup 3단계 권장 경로, tiers 비교표, researcher workflow end-to-end 예시

### 🐛 Bug Fixes
- **test**: `TestTryFix_BrokenSymlinkRemoved` — `tryFix` 후 새 CLAUDE.md 생성 반영하여 테스트 어서션 수정

### 🔧 Chores
- **skills**: `c4_notify` event param 추가 — plan/run/finish/c9 알림 이벤트 타입 명시
...
### 🐛 Bug Fixes
- **doctor**: C5 Hub 401 감지 시 `--fix`로 `cq auth login --device` 자동 실행
