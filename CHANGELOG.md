...
### v1.46 ✅ (2026-03-30)
- **feat(paper)**: Paper Mode — `cq paper` CLI + `/paper` skill for structured document/paper learning
  - `cq paper list/run`, BulkAddPapers, source-first add flow, auto-parsed title
  - Papers TUI screen in dashboard, source URL auto-open on session entry
- **feat(i18n)**: config-driven AI output language control (`config.yaml` → `language` 필드)
- **feat(watchdog)**: `cq serve --watchdog` — child process supervision with exponential backoff restart
- **feat(watchdog)**: Relay WebSocket restart — `cq relay call <worker> restart`
- **feat(worker)**: heartbeat self-healing circuit breaker (5 failures=warn, 10=restart)
- **feat(worker)**: `safeGo()` goroutine panic recovery + crash log remote upload (RingBuffer 500줄)
- **feat(hub)**: 429 adaptive backoff (Retry-After 존중, max 5min, 10회 재시도)
- **feat(orchestration)**: OrchestrationConfig + auto-routing (Direct/Quick/Full)
- **feat(orchestration)**: Double Loop TDD Engine — Verdict Engine, Lighthouse extension, 3-worker TDD pipeline
- **feat(craft)**: Ctrl+N to launch `/craft` session from Add screen
- **feat(jobs-tui)**: `cq jobs` bubbletea TUI — detail panel, adaptive multi-row charts, compare mode
- **feat(mcp-apps)**: MCP Apps widget system — 11 visual cards (Dashboard, Job Progress, Task Graph, etc.)
- **feat(ontology)**: L1-L3 persona ontology system (schema, store, updater, cross-position detector)
- **feat(standards)**: paper-review/paper-writing skills + 9-team CQ integration guide
...
- **fix(c5)**: non-GPU 워커가 GPU job 가져가는 버그 수정 (AcquireLease + ListPendingJobCandidates)
- **fix(c5)**: WSL2 nvidia-smi 감지 — /usr/lib/wsl/lib/ fallback 추가
- **feat(c5)**: job 성공/실패 모두 Dooray webhook 알림 (성공 시 메트릭 포함)

### v1.5.1 ✅ (2026-03-20)
- **feat(skills)**: Skills 2.0 마이그레이션 — 30개 스킬에 allowed-tools 추가, 4개 조회 스킬에 context:fork
- **feat(skilleval)**: BinaryEval 채점 엔진 + MutationEngine + SkillOptimizer 자율 최적화 루프
- **feat(c1)**: DoorayAdapter Hub polling 방식 전환 + Dooray pending queue 엔드포인트
- **fix(c5)**: Dooray pending/reply 인증 면제
...
- **skills**: `/finish` Step 7.7 — 세션 요약 → `c4_pop_extract(content=...)` 자동 주입
- **skills**: `/pi` → `/plan` 워크플로우 재설계 — EARS를 /pi로 병합, idea.md가 단일 리뷰 게이트
...
- **skills**: /pi 스킬 신설 — ideation 모드 (발산/수렴/심화/반론 4모드, /plan 자동 전환)
...
- **hooks**: 워크플로우 게이트 — `git commit`/`/finish` polish 미완료 차단, `C4_SKIP_GATE=1` 우회 지원
...
- **차별화**: `/run` = fire-and-forget 독립 Worker, `/c4-swarm` = TeamCreate+SendMessage 협업
