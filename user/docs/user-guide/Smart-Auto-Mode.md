...
> **v0.6.6 신규**: `/run`이 **Smart Auto Mode**를 기본으로 사용합니다.
...
```bash
/run 1         # 단일 Worker (디버깅용, 이 세션에서 직접 실행)
/run 3         # 3개 Worker 스폰
/run --max 4   # 자동 모드이지만 최대 4개로 제한
```
...
`/status` 또는 `/run` 실행 시 병렬도 분석 결과가 표시됩니다:
