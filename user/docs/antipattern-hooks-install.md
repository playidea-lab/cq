...
| `c4-gate.sh` | `.claude/hooks/` | C4 프로젝트 전용 | TodoWrite, TaskCreate, TaskUpdate, EnterPlanMode + 워크플로우 게이트 |
...
| 도구 | 차단 이유 | 대안 |
|------|---------|------|
| `TodoWrite` | C4 SSOT 이중 트래킹 | `c4_add_todo` 또는 `/add-task` |
| `TaskCreate` | C4 SSOT 이중 트래킹 | `c4_add_todo` (단일 소스: `.c4/tasks.db`) |
| `TaskUpdate` | C4 SSOT 이중 트래킹 | `c4_task_list` 확인 후 `c4_submit` |
| `EnterPlanMode` | C4 플래닝 워크플로우 우회 | `/plan` 스킬 |
| `git commit` | polish 미완료 + in_progress 태스크 | `/polish` 먼저 실행 |
| `/finish` | polish 미완료 | `/polish` 먼저 실행 |
...
bats c4-core/test/hooktest/test_c4gate_mcp.bats           # 10개 테스트
```
