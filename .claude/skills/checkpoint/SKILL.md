...
description: |
  ...
  "/checkpoint", "review checkpoint", "approve checkpoint",
...
> Checkpoint는 run에 내장. `run.checkpoint_mode: interactive` (기본)이면 이 스킬 수동 호출, `auto`면 Worker가 자동 리뷰.
...
| `auto_approve: false` | 사람이 `/checkpoint` 호출 필수 |
