...
description: |
  ...
  "run tasks", "execute plan", "spawn workers", "start implementation", "/run".
...
```
/run             # Continuous: spawn workers, auto-respawn until queue empty (default)
/run 3           # Continuous, initial batch 3 workers
/run --max 4     # Continuous, cap 4 workers per respawn
/run --single    # One round only (no respawn); then exit
```
...
**State routing**: PLAN/HALTED→Step 2, EXECUTE→Step 3, CHECKPOINT→config분기, COMPLETE→exit, INIT→"/plan first"
