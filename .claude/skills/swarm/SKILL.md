...
```
/swarm                  # Auto: C4 task-based team composition
/swarm 3                # Spawn 3 members
/swarm --review         # Review-only team (read-only)
/swarm --investigate    # Hypothesis competition mode
```
...
**Simple parallel → `/run`, team collaboration → `/swarm`.**
...
- `/run` — Independent Worker parallel (no communication)
